...
private:
  ...
  //  BUG FIX: original code multiplied difficulty twice for traps.
  //  Now: base probability is directly trap_probability * difficulty,
  //  corridors get slightly higher chance than open floors.
  void place_traps() {
    float corridor_prob = cfg_.trap_probability * cfg_.difficulty * 0.8f;
    float floor_prob = cfg_.trap_probability * cfg_.difficulty * 0.25f;
  ...
  // ── 3f. Enemies ────────────────────────────────────────
  //  BUG FIX: original default enemy_density was 0.8 (way too high).
  //  Now enemies are placed with proper spatial checking so the map
  //  doesn't look like a sea of red.
  void place_enemies() {
    bool ambush = (cfg_.enemy_type == "ambush");
    // probability per eligible tile
    float p = cfg_.enemy_density * cfg_.difficulty;
    std::uniform_real_distribution<float> roll(0.f, 1.f);
    ...
    for (int y = 0; y < map.H; ++y)
      for (int x = 0; x < map.W; ++x) {
        Tile &t = map.at(x, y);
        if (ambush) {
          // Ambush enemies prefer corridor junctions & room corners
          if (t == CORRIDOR && is_junction(x, y) && roll(rng_) < p * 0.6f)
            t = ENEMY;
          else if (t == FLOOR && is_corner(x, y) && roll(rng_) < p * 0.5f)
            t = ENEMY;
        } else {
          // Roaming enemies spread more evenly
          if ((t == FLOOR || t == CORRIDOR) && roll(rng_) < p * 0.25f)
            t = ENEMY;
        }
      }
  }
...
static RGB texture_pixel(Tile tile, int px, int py, int tx, int ty, int tsize,
  ...
  // ── WALL: dark stone with cracks ─────────────────────
  case WALL: {
    RGB base = dark_theme ? RGB{16, 14, 24} : RGB{72, 62, 52};
    RGB light = dark_theme ? RGB{32, 28, 46} : RGB{100, 88, 72};
    RGB crack = dark_theme ? RGB{8, 7, 12} : RGB{48, 40, 32};
    ...
    col.r = static_cast<uint8_t>(std::clamp(col.r + n2 * 20 - 10, 0.f, 255.f));
    col.g = static_cast<uint8_t>(std::clamp(col.g + n2 * 20 - 10, 0.f, 255.f));
    col.b = static_cast<uint8_t>(std::clamp(col.b + n2 * 20 - 10, 0.f, 255.f));
    return col;
  }

  // ── FLOOR: worn stone with subtle tile pattern ────────
  case FLOOR: {
    RGB base = dark_theme ? RGB{50, 45, 68} : RGB{190, 175, 150};
    RGB grout = dark_theme ? RGB{30, 27, 42} : RGB{140, 128, 108};
    ...
    col.r = static_cast<uint8_t>(
        std::clamp(col.r * (0.85f + wear) + n2 * 15, 0.f, 255.f));
    col.g = static_cast<uint8_t>(
        std::clamp(col.g * (0.85f + wear) + n2 * 15, 0.f, 255.f));
    col.b = static_cast<uint8_t>(
        std::clamp(col.b * (0.85f + wear) + n2 * 15, 0.f, 255.f));
    return col;
  }

  // ── CORRIDOR: rougher, narrower stone ────────────────
  case CORRIDOR: {
    RGB base = dark_theme ? RGB{34, 30, 50} : RGB{140, 130, 112};
    RGB dark2 = dark_theme ? RGB{20, 18, 30} : RGB{100, 92, 78};
    float f = fbm(gx * 0.6f, gy * 0.6f, 3, 3);
    RGB col = lerp_rgb(dark2, base, f);
    col.r = static_cast<uint8_t>(std::clamp(col.r + n2 * 18 - 8, 0.f, 255.f));
    col.g = static_cast<uint8_t>(std::clamp(col.g + n2 * 18 - 8, 0.f, 255.f));
    col.b = static_cast<uint8_t>(std::clamp(col.b + n2 * 18 - 8, 0.f, 255.f));
    return col;
  }
